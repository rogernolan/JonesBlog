...
3. Before modifying files, inspect the current branch and confirm that it matches the ticket or feature being worked on. Create a new ticket-specific feature branch or worktree from the latest `origin/main` unless Rog/Jane explicitly says otherwise; do not reuse an unrelated or stale existing feature branch.
