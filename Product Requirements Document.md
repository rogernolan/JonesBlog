...
Blogger - one of a small group of people who all create BlogItems to contribute to a shared Blog. In v1, all Bloggers are equal owners of the Blog, and may perform all functions on the shared Blog. In v1, the two coders will be the only Bloggers in the BloggerGroup. Bloggers have a displayName which they can edit.

BloggerGroup - the set of Bloggers who are allowed to create BlogItems in a particular Blog.

BlogItem - the unit created by one person as a contribution to the shared narrative. A BlogItem has optional text and photo but must have at least text or a photo. A BlogItem also has a datetime, initially set to its creation date. When a photo is added to a BlogItem from the photo library, the datetime can optionally be updated to the datetime of the photo. The BlogItem's date can be changed by the user. Part of the underlying data model.

Caption - a paragraph of text. Part of a BlogItem
...
DayPost - all the top level BlogItems and Galleries made (or dated) for one particular day. For V1 DisplayItem is a display artefact which can be constructed when needed from BlogItems.

Trip - all the DayPosts for a certain time period (usually weeks or months), set by a Blogger. A Blogger will usually start an open Trip, to which all DayPosts are assigned, until a Blogger closes the Trip. If a BlogItem's date changes such that it is within a different DayPost, then it should move to that DayPost. In effect, DayPosts are constructed on the fly by finding all BlogItems and Galleries with a certain local calendar date, and a Trip is created on-the-fly by finding DayPosts within a range of local calendar dates. A Trip exists in the data model as a start local date and possibly an end local date. The viewable Trip, with all its DayPosts and their BlogItems is constructed on the fly as the Trip is viewed. A trip also has a Title, a Description and a heroImage. 
...
Settings - a Blog has settings
- GalleryInterval - the time difference inside which galleries are created. It will default to 15 minutes.
- GalleryDistance - the spatial distance within which galleries are created. It will default to 500m.
...
- Make it effortless for a group to collectively document an experience as it happens, with minimal manual input.
...
- As a Blogger, I can create a new BlogItem with a single photo and/or a text caption, so I can quickly capture a moment.
- As a Blogger, when I create a BlogItem, the app automatically tags it with my current location — reverse geocoded to a human-readable city name and a country code — so I do not have to type where I am.
- As a Blogger, when I create an entry, the app automatically fetches current weather conditions and attaches a temperature and appropriate weather conditions, so the context is captured without effort.
...
- The InstaBlog app automatically groups all entries from the same calendar day into a single DayPost. Entries are ordered by time.
...
- DayPosts are grouped by the BlogItem’s local calendar date. A BlogItem’s local date is determined from its location timezone when available; otherwise from the device timezone at creation/edit time. When a BlogItem’s date, time, or location changes, its local calendar date may change and it moves to the corresponding DayPost.
- Each BlogItem stores enough date context to reconstruct its local day: absolute datetime, timezone identifier when known, and optional location.
...
- In this Trip list, galleries are displayed as horizontally scrolling lists within a single list entry
...
- There are also smaller, less prominent + buttons within a Trip to insert a new BlogItem between two adjacent BlogItems. The date and time for this BlogItem will be halfway between its neighbours. 
- Tapping a BlogItem opens a full view showing the photo and/or caption, location, and weather. An edit control, when tapped, allows any Blogger to edit the BlogItem or delete it.
- Tapping a Gallery shows the multiple BlogItems that make it up in a vertical list similar to the main trip list with similar date and detail view accessed through a tap. If all BlogItems in a Gallery are deleted, the Gallery is deleted.
...
- Each instance of the app can only view and edit one Blog at once. This can either be a blog created by the user or a blog shared to this instance of the app. 
- A Blogger may leave a shared Blog and return to editing their own local blog or go on to accept another share request.
- A Blogger who is sharing a Blog may turn sharing off, removing all other Bloggers and reverting the blog to unshared.
...
- All BlogItems and DayPosts are visible to all Bloggers in the BloggerGroup and they sync automatically.
- All persisted Blog data — BlogItems, Trips, locations, weather, media, settings, and subscriber list — is shared across Bloggers. There is no private content within a blog. Galleries are derived from shared BlogItems and settings.
...
- As a Blogger, I can send a DayPost to the blog's mailing list, the SubscriberMailList. The app composes an HTML email via native Apple frameworks addressed to all subscribers, ready to send.
...
- **Group management:** Mostly resolved for v1. Invitation mechanism, recovery edge cases, and exact warning text remain open.
- **Static blog format:** Future work, not in scope for v1.
- **Mailing list integration:** Resolved — the blog maintains its own subscriber list stored in the model and shared across all authors. Any contributor can manage subscribers or send a DayPost. Emails are sent as HTML via Apple native frameworks. No third-party mail provider required.
- **Video support:** No video for V1

## Future Considerations

These are not v1 requirements. The v1 design should not add major implementation work for them, but it should avoid choices that would make them unnecessarily difficult later.

- **Video support:** BlogItems may later support video as a third content type alongside text and photo.
- **First-class Galleries and DayPosts:** Galleries and DayPosts may later become persisted objects with their own metadata, ordering, captions, or publishing state rather than purely derived display artifacts.
- **Finer-grained permissions:** A later public version may distinguish owners, contributors, editors, and viewers instead of giving all Bloggers equal permissions.
- **Rich text formatting:** BlogItems may support rich text and links.
- **iOS sharing:** Bloggers may share content to and from InstaBlog.
