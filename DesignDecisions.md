...
The `InstaBlog Live` Xcode scheme checks that CloudKit Development and Production schemas match before its Release archive. Routine development builds do not run this archive-only check.
...
The legacy Development-to-Production migration is complete. The temporary **InstaBlog Migration Export** scheme and its build variant, which exported pre-isolation Development data through the legacy `com.jonesthevan.blog.InstaBlog` bundle identifier with CloudKit Development, were removed after the transfer finished (issue #374).
