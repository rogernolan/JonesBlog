...
All three App IDs use `iCloud.com.jonesthevan.blog.InstaBlog`. A fresh side-by-side installation has independent local-only and CloudKit-backed SQLite databases. At launch, a valid CloudKit-delivered Blog cached in the CloudKit-backed database is selected immediately unless the local-only store has unadopted entries for that root; those entries keep the local journal selected only until the arrival/adoption workflow has attempted their structured copy and switched to the cached root. A Blog row alone is insufficient: its SyncEngine metadata must prove that the root was written by CloudKit. When no valid cached root exists, bootstrap opens the permanent local-only journal immediately and does not start CloudKit synchronization or attach CloudKit metadata to that database. This protects offline first use—including a device awaiting an accepted share—from accidental upload. A later connectivity-driven sync, remote-root detection, local-content adoption, and workspace switch are handled by the arrival/adoption workflow. The future user-initiated promotion of a standalone journal to a new CloudKit journal is tracked separately. Explicit sharing and media synchronization requests use one serialized gate so they cannot concurrently submit the same structured CloudKit records.

On connectivity, the arrival workflow explicitly synchronizes the separate CloudKit-backed store. A valid CloudKit-delivered root is detected silently; only then does the app announce the download. It keeps the local journal visible while it attempts every local Blogger, BlogItem, PhotoItem, and MediaAsset copy with fresh UUIDs using a local idempotency ledger. Local trips, lists, and subscribers are not copied. The app then switches to the received root without modifying its title, trips, lists, subscribers, or existing entries, whether the structured copy succeeded or reported failures. Any failure is captured with individual record details, shown in an apology modal, and leaves the original local journal intact for recovery while the CloudKit download continues in the background. Structured entries are displayed as soon as the workspace switches; original media files copy and synchronize afterwards and do not block the structured journal.

### Cross-Environment Blog Archive

The manual `.instablogarchive` transfer format exists for one-time Development-to-Production recovery. It contains a versioned JSON manifest and original media files. It preserves timestamps and record relationships, but every imported domain record receives a new UUID. It deliberately excludes SQLiteData sync metadata, CloudKit record/asset identifiers, sync errors, local paths, and CloudKit share metadata.

Import validates the archive version, record relationships, filenames, media counts, and SHA-256 hashes before presenting the destructive confirmation. After explicit **Erase and Import** confirmation, the app stops the current sync engine, closes the database, and releases the runtime; deletes the SQLite database, SQLiteData metadata, media directory, private CloudKit zone, and accepted/shared CloudKit zone; then explicitly recreates the empty private zone and creates a fresh database. While that database is still empty, the app starts SQLiteData only long enough to acknowledge the committed zone transition, then stops it again. This reset barrier prevents a late zone-deletion event from erasing imported local records. The archive import itself performs no CloudKit synchronization or sharing: the stopped SyncEngine's installed triggers queue every imported record as an unsynchronized local record. Imported media is stored under its content hash. A subsequent app launch explicitly starts normal private synchronization, and sharing must be created again by the user.
...
Other database preparation failures show a recoverable startup screen and are captured in Sentry with the underlying error. Retrying reopens and prepares the existing database. Debug and Live Debug additionally offer an explicitly confirmed **Reset Database** action that stops synchronization, closes and deletes this build's local database, sync metadata, and media, deletes its CloudKit records and shares, and bootstraps a new empty Blog under a paused SyncEngine. Normal private synchronization starts explicitly on the next app launch; because the paused engine's triggers tracked the replacement records, an empty production zone causes an upload rather than deleting the local Blog. Release and TestFlight do not compile this reset action, and startup never deletes data automatically.
...
To recover the pre-isolation Development data, select the temporary **InstaBlog Migration Export** scheme and run it on the device that still holds that Development data. If TestFlight has since replaced the old local installation, first confirm that its production data has synchronized, delete TestFlight, and install Migration Export fresh so it downloads only Development data rather than reusing production SQLiteData metadata. This scheme deliberately uses the legacy `com.jonesthevan.blog.InstaBlog` bundle identifier with CloudKit Development, so it cannot coexist with TestFlight on that device. Export the archive from Settings before reinstalling TestFlight. In Live Debug, choose **Erase and Import**, confirm the imported records and photos remain editable offline, allow the normal private CloudKit upload to happen afterward, and then recreate the production CloudKit share manually. Never use the migration scheme after the transfer is complete.
